...
prompt5 = PromptTemplate.from_template(
    '다음 속박 시설 리뷰 요약에 대해 공손한 답변을 작성하세요.\n답변언어:{language}\n리뷰 요약{summary}'
)

# 프롬프트 6 : 생성 답변을 한국어로 번역
prompt6 = PromptTemplate.from_template(
    '당음 생성된 답연을 한국어로 반역 해주세요.\n리뷰 반역 {reply1}'
)

# ---LCEL을 사용한 체인 구성요소 정의--
# 단계 1 :리뷰 번역 체인
...
combined_lcel_chain = (
    RunnablePassthrough.assign(
        # 입력 -> {'review': '원본 리뷰 텍스트'}
        # 출력 -> {'review':'....', 'translation': 'qksdurehls xprtmxm'}
        translation = lambda x:translate_chain_component.invoke({'review' : x['review']})
    )
    | RunnablePassthrough.assign(
        # 입력 -> {'review':'.......', 'translation':'번역된 텍스트'}
        # 출력 -> {'review':'.....', 'translation': '...',
        #      'summary': '요약된 텍스트','sentiment_score': '감성 점수',
        #      'language': '언어'
